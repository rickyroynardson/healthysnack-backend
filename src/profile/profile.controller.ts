...
import { getProfile, updatePassword, updateProfile } from "./profile.service";
import {
...
profileController.patch(
  ...
  async (req: RequestWithUser, res: Response) => {
    const result = await updatePasswordValidation.safeParseAsync(req.body);
    if (!result.success) {
      return res
        .status(422)
        .json({ message: fromZodError(result.error).message });
    ...
    try {
      const profile = await updatePassword(req.user?.id!, result.data);
      res
        .status(200)
        .json({ message: "Password updated successfully", data: profile });
    } catch (error) {
      if (error instanceof Error) {
        res.status(400).json({ message: error.message });
      }
